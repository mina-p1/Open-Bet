...
if 'gameId' in df_games.columns: df_games['gameId'] = df_games['gameId'].apply(clean_id)
...
if 'gameDateTimeEst' in df_stats.columns:
    df_stats['gameDateTimeEst'] = pd.to_datetime(df_stats['gameDateTimeEst'], utc=True, errors='coerce')
...
df_stats = df_stats.drop_duplicates(subset=['gameId', 'teamId'], keep='first')
...
cols_to_fill = ['fieldGoalsAttempted', 'freeThrowsAttempted', 'reboundsOffensive', 'turnovers']
for col in cols_to_fill:
    if col not in df_stats.columns: df_stats[col] = 0
...
# Home court advantage logic
df_stats['point_margin'] = df_stats['teamScore'].astype(float) - df_stats['opponentScore'].astype(float)
df_stats['home_margin_only'] = df_stats.apply(lambda x: x['point_margin'] if x.get('home', 0) == 1 else np.nan, axis=1)
df_stats['home_strength_rating'] = df_stats.groupby('teamId')['home_margin_only'].apply(
    lambda x: x.expanding().mean().shift().ffill()
).reset_index(level=0, drop=True).fillna(0)

# Rolling Stats over past 10 gmaes
features_to_roll = ['teamScore', 'opponentScore', 'fieldGoalsPercentage', 'possessions', 'reboundsTotal', 'assists']
for f in features_to_roll:
    if f not in df_stats.columns: df_stats[f] = 0.0

rolling_df = df_stats.groupby('teamId')[features_to_roll].apply(
    lambda x: x.rolling(window=10, min_periods=1).mean().shift()
...
opponent_stats = df_stats_only.rename(columns={
    'teamId': 'opp_teamId',
...
for c in feature_cols:
    if c not in full_data.columns: full_data[c] = 0.0
